...
describe "Travis Migrations Custom Rake Tasks" do
  ...
  it "setups the database" do
    expect(system('RAILS_ENV=test rake db:drop')).to be_truthy
    expect(system('RAILS_ENV=test rake db:create')).to be_truthy
    expect(system('RAILS_ENV=test rake db:migrate')).to be_truthy
